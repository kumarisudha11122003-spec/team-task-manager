          ...
          <header className="header-light h-[64px] shrink-0 flex items-center justify-between px-6 z-50">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative" ref={bellRef}>
                ...
                {showNotifications && (
                  <div className={`absolute top-full -right-4 sm:right-0 mt-2 w-[280px] sm:w-[320px] max-w-[90vw] rounded-2xl shadow-xl border ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-black/5'} z-50 overflow-hidden`}>
              ...
              <div className="relative" ref={avatarRef}>
                ...
                {showUserMenu && (
                  <div className={`absolute top-full -right-2 sm:right-0 mt-2 w-48 rounded-2xl shadow-xl border ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-black/5'} z-50 overflow-hidden`}>
                    <div className="p-3 border-b border-inherit">
                      <div className="font-bold text-sm truncate">{userName}</div>
                      <div className="text-xs text-slate-400 truncate">{userEmail}</div>
                    </div>
                    <div className="p-2">
                      <button onClick={() => { setShowUserMenu(false); navigate('/settings'); }} className={`w-full text-left px-3 py-2 text-sm rounded-xl transition-colors ${isDark ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-100 text-[#1a1a2e]'}`}>
                        Profile Settings
                      </button>
                      <button onClick={handleLogout} className="w-full text-left px-3 py-2 text-sm rounded-xl transition-colors text-[#FF3D71] hover:bg-[#FF3D71]/10 mt-1">
                        Logout
                      </button>
                    </div>
                  </div>
